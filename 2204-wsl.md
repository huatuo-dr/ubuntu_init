...
git clone https://github.com/huatuo-dr/tmux.conf
...
# 打开一次nvim 等待插件安装完成
rm -rf ~/.config/nvim
# git@github.com:huatuo-dr/nvim.git
git clone https://github.com/huatuo-dr/nvim.git -b ver-0.12.0 ~/.config/nvim
# 按照readme配置
...
```shell
# git
alias gitperson='git config --global user.name "huatuo-dr"; git config --global user.email "dlkxiaok@163.com"'
...
# import 后用 root 进入实例，手动创建新用户、设置密码、设置默认用户。
# 假设实例名是 u2204：
wsl --import u2204 D:\WSL\u2204 D:\WSL\images\ubuntu-22.04.tar --version 2

# 然后用 root 进入：
wsl -d u2204 -u root

# 在 Ubuntu 里执行，假设新用户名叫 newuser：
adduser newuser
...
# 这时默认就是 newuser 登录。
# 如果你不想保留原来的 huatuo 用户，可以在 root 下删除：
deluser --remove-home huatuo
```
