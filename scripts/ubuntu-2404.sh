...
require_ubuntu_2404() {
    ...
    if [[ "${ID}" != "ubuntu" || "${VERSION_ID}" != "24.04" ]]; then
        die "This script should run on Ubuntu 24.04."
    fi
}
...
setup_apt_sources() {
    info "Configuring Aliyun apt sources for Ubuntu 24.04"

    if [[ -f "${APT_SOURCES_FILE}.bak" ]]; then
        warn "Apt sources backup already exists, keeping it: ${APT_SOURCES_FILE}.bak"
    else
        sudo cp "${APT_SOURCES_FILE}" "${APT_SOURCES_FILE}.bak"
    fi

    sudo tee "${APT_SOURCES_FILE}" >/dev/null <<'EOF'
Types: deb
...
install_tree_sitter_cli() {
    ...
    info "Installing tree-sitter-cli for Ubuntu 24.04"
...
main() {
    ...
    info "Ubuntu 24.04 WSL initialization starts"
    ...
    success "Ubuntu 24.04 WSL initialization finished"
}
